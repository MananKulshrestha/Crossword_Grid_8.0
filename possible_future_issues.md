...
- Gemma 4 26B A4B is configured as an alias, not a hard-coded business assumption. The serving provider, tokenizer, structured-output support, thinking-mode defaults, context window, quantization, and license must be verified before changing the alias in a release tuple.
- The Google Gemini adapter must keep its provider-safe `responseJsonSchema` projection separate from the full Pydantic contract. Sending `$defs`, `$ref`, discriminators, or unsupported schema keywords directly can produce provider `INVALID_ARGUMENT` responses; omitting post-response validation would allow malformed intent to route actions.
- A live hosted Gemma 4 26B intent smoke returned valid typed JSON in about 2.4 seconds after disabling thinking, above the 1.8-second shopper budget. This currently produces the intended safe interpretation fallback under the production cap; a future serving choice or approved latency decision is needed before claiming the 26B path meets the target.
