...
# Inlined from the RA repo's graph_sampling.py rather than imported across
# repos -- this fkgrid package and RA are two separate codebases now that
# this file lives under src/fkgrid/db/, so a sys.path hack back into RA
# would be fragile. Keep this in sync with RA/graph_sampling.py's version
# if that one changes; same family-grouping purpose, just duplicated.
...
def normalize_product_name(name):
    if not name:
        return ""
    tokens = [t for t in _WORD_RE.findall(name.lower()) if t not in _COLOR_WORDS]
    return " ".join(tokens)
