...
export interface Session { session_id: string; state_version: number; cart_version: number; cart: Cart; acknowledged_entries: Entry[]; query_state: { hard_constraints: { field_id: string; values: unknown[] }[]; soft_preferences: { field_id: string; values: unknown[] }[] }; model: { speech_configured: boolean; configured: boolean; cart_ready: boolean } }
