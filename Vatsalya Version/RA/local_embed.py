...
Moves embedding calls off the shared GPU cluster entirely -- they were
competing with concurrent gemma4:31b extraction calls for the same
physical GPUs, causing embedding requests to blow past LightRAG's 60s
worker timeout under load (see IMPLEMENTATION.md). Extraction (the LLM
calls) stays on Ollama; only embedding moves here.
...
close to the Ollama-served embeddings already in lightrag_storage/, but
not guaranteed byte-identical (different serving stack, no GGUF
quantization here). Matches the current no-prefix behavior of
ingest.py's existing Ollama embedding call (neither uses the
"search_query: "/"search_document: " asymmetric prefixes Nomic
recommends) so switching backends mid-corpus doesn't also change
whatever's already been embedded relative to what's embedded from here on.
...
# asyncio.to_thread (separate OS threads) caused severe slowdowns (30+
# seconds for a single short embed, vs 0.24s measured with one call at a
# time) -- serializing is both correct and faster here, unlike the
# Ollama backend where concurrency helps by overlapping network latency.
