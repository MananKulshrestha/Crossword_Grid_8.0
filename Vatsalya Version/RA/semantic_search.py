...
Does not modify ingest.py or query.py. This is a second, independent caller
of ingest.build_rag() -- exactly what query.py already is -- so both files
keep working standalone, unchanged.
...
def search(query_text: str, top_n: int = 50) -> list[Candidate]:
    ...
    return asyncio.run(_search_async(query_text, top_n))
