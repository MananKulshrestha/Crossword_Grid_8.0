...
23. **No-safe versus clarification semantics.** A planner `NO_SAFE_RECOVERY` is an explicit abstention and must not be converted into a generic category question merely because a broad allowlist happens to contain two concepts. Only an explicit clarification or a deterministic provider/validation fallback with relevant options should produce “Did you mean …?”; otherwise return honest no-match.
24. **Retrieval adapter result integrity.** Every Tier 1, cached, and Tier 2 run must carry the exact rewritten query-state hash, hard-filter hash, compatibility tuple, and safe-scope diagnostics. Checking only the hard-filter hash can accept results produced for the wrong query.
25. **Planner telemetry privacy and cardinality.** The planner input hash, token count, latency, and allowlist count are safe bounded telemetry, but raw context, shopper terms, model output, and provider bodies must stay out of events and metrics. Do not turn labels or IDs into unbounded metric dimensions.
